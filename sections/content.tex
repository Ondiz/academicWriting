...
\note{I'm sharing here some tools and tricks I use. We've talked about
  Overleaf before, I use it mainly for searching template. Then we
  have Inkscape that lets us export the text as LaTeX so that the font
  is the same as the one in the document. Also, as I mentioned, tables
  are a pain, there is this Excel plugin to export them from
  Excel. Then my two favourite tools: Detexify, that finds a symbol
  that we write and Pandoc, a command line tool to convert from any
  format to another so we can always write in LaTeX
  (EXAMPLE). Finally, we have LyX, that's an editor similar to Word
  but that uses LaTeX in the background, great to learn!

  Testing: \href{http://mathb.in}{MathB.in};
